...
class MobileSeriesControllerTest {

    @Mock
    private MobileSeriesService mobileSeriesService;

    @InjectMocks
    private MobileSeriesController controller;

    @Test
    void getSeries_defaultParams_delegatesCorrectly() {
        MobilePageResponse<MobileSeriesSummary> expected = buildSeriesPage();
        when(mobileSeriesService.getSeries(0, 20, "recentlyAdded", "desc", null, null, false))
                .thenReturn(expected);

        ResponseEntity<MobilePageResponse<MobileSeriesSummary>> response =
    ...
    void getSeriesBooks_defaultParams_delegatesCorrectly() {
        MobilePageResponse<MobileBookSummary> expected = buildBookPage();
        when(mobileSeriesService.getSeriesBooks("Dune", 0, 20, "seriesNumber", "asc", null))
                .thenReturn(expected);

        ResponseEntity<MobilePageResponse<MobileBookSummary>> response =
    ...
    private MobilePageResponse<MobileSeriesSummary> buildSeriesPage() {
        MobileSeriesSummary summary = MobileSeriesSummary.builder()
                ...
                .coverBooks(List.of())
                .build();
        return MobilePageResponse.of(List.of(summary), 0, 20, 1);
    }

    private MobilePageResponse<MobileBookSummary> buildBookPage() {
        MobileBookSummary summary = MobileBookSummary.builder()
                .id(1L)
                .title("Test Book")
                .authors(List.of("Author A"))
                .seriesName("Dune")
